...
while True:
    ...
    print(" Checking..")
    status_api = ping_ip(api_server)
    print(f"  {Fore.YELLOW}api{Style.RESET_ALL}.playit.gg: {Fore.GREEN}Online{Style.RESET_ALL}" if status_api else f"  {Fore.YELLOW}api{Style.RESET_ALL}.playit.gg: {Fore.RED}Offline{Style.RESET_ALL}")

    status_debug_ip = ping_ip(debug_ip)
    isp_message = " (Your ISP might be blocking Playit's IPs!)" if not status_debug_ip else ""
    print(f"  Debug {Fore.YELLOW}IP{Style.RESET_ALL}: {Fore.GREEN}Online{Style.RESET_ALL}" if status_debug_ip else f"  Debug {Fore.YELLOW}IP{Style.RESET_ALL}: {Fore.RED}Offline{Style.RESET_ALL}{isp_message}")
    print("")
